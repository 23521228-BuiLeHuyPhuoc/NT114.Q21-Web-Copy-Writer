...
export function CustomerContentDetail() {
  ...
  return (
    <Layout>
      <div className="p-6 max-w-5xl mx-auto">
        {/* Back */}
        <Button variant="ghost" className="mb-4 text-gray-600" onClick={() => navigate('/contents')}>
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <Badge className="bg-green-100 text-green-700 border-0">Đã xuất bản</Badge>
              <Badge className="bg-gray-100 text-gray-600 border-0">{content.type}</Badge>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            <Card className="p-6">
              ...
              <div className="bg-gray-50 rounded-xl p-5 border text-sm text-gray-800 leading-relaxed">
            ...
            {/* Versions */}
            <Card className="p-6">
              <h3 className="font-semibold text-gray-900 mb-4">Phiên bản</h3>
              <div className="flex gap-3">
                {content.versions.map(v => (
                  <div key={v.id} className={`flex-1 p-3 rounded-xl border-2 cursor-pointer transition-all ${v.selected ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-gray-300'}`}>
                    <p className="text-sm font-semibold text-gray-900">{v.label}</p>
                    <p className="text-xs text-gray-500 mt-1">Chất lượng: <span className="text-green-600 font-semibold">{v.quality}%</span></p>
                  </div>
                ))}
              </div>
            </Card>
            ...
            <Card className="p-6 bg-gradient-to-r from-green-50 to-emerald-50 border-green-200">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-semibold text-gray-900 mb-1">Tạo lại nội dung</h3>
                  <p className="text-sm text-gray-600">Tạo phiên bản mới với cùng cấu hình hoặc chỉnh sửa tham số</p>
                </div>
          ...
          <div className="space-y-4">
            ...
            <Card className="p-5">
              <h3 className="font-semibold text-gray-900 mb-3">Điểm chất lượng</h3>
              <div className="text-center py-4">
                <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-gradient-to-br from-green-100 to-emerald-100 mb-3">
                  <span className="text-2xl font-bold text-green-700">{content.quality}%</span>
                </div>
            ...
            <Card className="p-5">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900">{content.createdAt}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Cập nhật</span>
                  <span className="text-gray-900">{content.updatedAt}</span>
                </div>
              </div>
            </Card>
          </div>
