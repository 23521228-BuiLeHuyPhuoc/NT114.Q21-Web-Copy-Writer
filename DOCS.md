# AI Copywriter – Tài Liệu Kỹ Thuật Chi Tiết (Technical Documentation)
...
- **Mô tả:** Tài liệu kỹ thuật chi tiết cho hệ thống AI Copywriter – website tích hợp GPT-4 / Llama để sinh nội dung tự động (blog, quảng cáo, email marketing, mô tả sản phẩm, …). Hệ thống cung cấp API RESTful cho AI model xử lý nội dung trên backend, đồng thời hỗ trợ fine-tuning (tinh chỉnh model) để phù hợp với ngành nghề cụ thể. Ngoài ra, hệ thống tích hợp **8 tính năng nâng cao ⭐** bao gồm:
  - **RAG** (Retrieval-Augmented Generation) – sinh nội dung dựa trên tài liệu tham khảo
  - **Real-time Collaboration** – chỉnh sửa nội dung cùng lúc nhiều người (CRDT)
  - **NLP Content Analytics** – phân tích chất lượng nội dung bằng NLP
  - **AI Plagiarism Detection** – kiểm tra đạo văn tự động
  - **Elasticsearch Full-text Search** – tìm kiếm toàn văn hỗ trợ tiếng Việt
  - **Redis Caching & Session** – caching, rate limiting, session management
  - **BullMQ Background Jobs** – xử lý tác vụ nặng bất đồng bộ
  - **CI/CD Pipeline** – tự động lint, test, build, deploy qua GitHub Actions
  - *(Xem chi tiết kỹ thuật tại [Mục 7 – Tính Năng Nâng Cao](#7-tính-năng-nâng-cao-advanced-features-))*
